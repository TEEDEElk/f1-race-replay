...
# Anime Font Configuration
# Using Windows system fonts that have an anime/stylized look
ANIME_FONT_PATH = os.path.join(os.path.dirname(__file__), "..", "resources", "fonts", "anime.ttf")
# Fallback fonts in order of preference (Comic Sans for anime style, then Calibri, then default)
if os.path.exists(ANIME_FONT_PATH):
    ANIME_FONT = ANIME_FONT_PATH
else:
    ANIME_FONT = "Comic Sans MS"  # Anime-like appearance on Windows
ANIME_FONT_SIZE_LARGE = 32
ANIME_FONT_SIZE_MEDIUM = 24
ANIME_FONT_SIZE_SMALL = 16
...
def generate_f1_engine_sound(speed_kmh, duration=0.1, sample_rate=44100):
    """
    Generate a simple F1 engine sound based on speed using sine waves.
    Higher speed = higher pitch and more aggressive sound.
    """
    try:
        # Normalize speed to frequency range (500 Hz to 5000 Hz)
        min_speed = 0
        max_speed = 350
        speed_normalized = max(0, min(1, (speed_kmh - min_speed) / (max_speed - min_speed)))
        
        # Base frequency (fundamental engine tone)
        base_freq = 500 + speed_normalized * 4500
        
        # Create samples for the duration
        num_samples = int(sample_rate * duration)
        t = np.linspace(0, duration, num_samples, endpoint=False)
        
        # Create complex engine sound with harmonics
        wave1 = np.sin(2 * np.pi * base_freq * t)  # Fundamental
        wave2 = 0.3 * np.sin(2 * np.pi * base_freq * 1.5 * t)  # Harmonic
        wave3 = 0.2 * np.sin(2 * np.pi * base_freq * 2 * t)  # Second harmonic
        
        # Add engine roughness/noise
        noise = np.random.normal(0, 0.1, len(t))
        
        # Combine all components
        engine_sound = (wave1 + wave2 + wave3) * 0.4 + noise * 0.2
        ...
        engine_sound = np.clip(engine_sound, -1, 1)
        audio_data = (engine_sound * 32767).astype(np.int16)
        
        # Return the raw audio bytes
        return audio_data.tobytes()
    except Exception as e:
        print(f"Error generating F1 engine sound: {e}")
        return None
...
class F1ReplayWindow(arcade.Window):
    ...
    def on_draw(self):
        ...
        arcade.Text(lap_str,
                          20, self.height - 40, 
                          arcade.color.WHITE, ANIME_FONT_SIZE_LARGE, anchor_y="top", bold=True).draw()
        
        arcade.Text(f"Race Time: {time_str} (x{self.playback_speed})", 
                         20, self.height - 80, 
                         arcade.color.WHITE, ANIME_FONT_SIZE_MEDIUM, anchor_y="top", bold=True).draw()
        
        if current_track_status == "2":
            status_text = "YELLOW FLAG"
            arcade.Text(status_text, 
                             20, self.height - 120,
                             arcade.color.YELLOW, ANIME_FONT_SIZE_LARGE, bold=True, anchor_y="top", font_name=ANIME_FONT).draw()
        elif current_track_status == "5":
            status_text = "RED FLAG"
            arcade.Text(status_text, 
                             20, self.height - 120, 
                             arcade.color.RED, ANIME_FONT_SIZE_LARGE, bold=True, anchor_y="top", font_name=ANIME_FONT).draw()
        elif current_track_status == "6":
            status_text = "VIRTUAL SAFETY CAR"
            arcade.Text(status_text, 
                             20, self.height - 120, 
                             arcade.color.ORANGE, ANIME_FONT_SIZE_LARGE, bold=True, anchor_y="top", font_name=ANIME_FONT).draw()
        elif current_track_status == "4":
            status_text = "SAFETY CAR"
            arcade.Text(status_text, 
                             20, self.height - 120, 
                             arcade.color.BROWN, ANIME_FONT_SIZE_LARGE, bold=True, anchor_y="top", font_name=ANIME_FONT).draw()
        ...
        arcade.Text("Leaderboard", leaderboard_x, leaderboard_y, 
                         arcade.color.WHITE, ANIME_FONT_SIZE_MEDIUM, bold=True, anchor_x="left", anchor_y="top", font_name=ANIME_FONT).draw()
    ...
    def _update_engine_sound(self):
        """Update F1 engine sound based on average car speeds in current frame."""
        pass

    def _play_tone(self, frequency, duration=0.1, volume=0.5, sample_rate=44100):
        """Generate and play a sine wave tone at the specified frequency."""
        pass
